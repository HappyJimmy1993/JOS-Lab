	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	struct Env* env_s, * env_d;
	if(envid2env(srcenvid, &env_s, 1)||
		envid2env(dstenvid, &env_d, 1))
		return -E_BAD_ENV;
		
	pte_t* pte;
	struct Page* page = page_lookup(env_s->env_pgdir, srcva, &pte);
	if(!page)
		return -E_INVAL;
	...
	if(page_insert(env_d->env_pgdir, page, dstva, perm))
		return -E_NO_MEM;
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	if(env->env_ipc_from) return -E_IPC_NOT_RECV;
	...
	if((uintptr_t)srcva<UTOP){
		...
		pte_t* pte;
		struct Page* page;
		if(!(page = page_lookup(curenv->env_pgdir, srcva, &pte))) return -E_INVAL;
...
static int
sys_env_set_pl(envid_t envid, int pl)
{
	struct Env* env;
