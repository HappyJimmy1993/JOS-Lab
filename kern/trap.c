...
static inline void wrmsr(unsigned int msr, unsigned low, unsigned high)
{
...
trap_init_percpu(void)
{
	...
	int gdtOffset = GD_TSS0 + (cpuID<<3);
	int gdtIndex = gdtOffset>>3;
	gdt[gdtIndex] = SEG16(STS_T32A, (uint32_t) &thiscpu->cpu_ts, sizeof(struct Taskstate), 0);
	gdt[gdtIndex].sd_s = 0;
...
page_fault_handler(struct Trapframe *tf)
{
	...
	// LAB 3: Your code here.
	if((tf->tf_cs & 3) == 0){
		panic("Kernel model page fault: %08x", fault_va);
	}
	...
	if(curenv->env_pgfault_upcall){
		...
		uint32_t stktop = (tf->tf_esp >= UXSTACKTOP - PGSIZE &&
							tf->tf_esp < UXSTACKTOP) ?
							tf->tf_esp - sizeof(struct UTrapframe) - 4 :
							UXSTACKTOP - sizeof(struct UTrapframe);
					
		user_mem_assert(curenv, (void*) stktop, sizeof(struct UTrapframe), PTE_U|PTE_W);
