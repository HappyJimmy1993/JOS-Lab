...
boot_alloc(uint32_t n)
{
	...
	if (n>0)
	{
		uint32_t roundup_n = ROUNDUP(n, PGSIZE);
		if (PADDR(nextfree + roundup_n) > (npages * PGSIZE))
		{
...
page_alloc(int alloc_flags)
{
	...
	struct Page *tem = page_free_list;
...
page_alloc_npages(int alloc_flags, int n)
{
	...
	int i;
	if (n <= 0) return NULL;
	struct Page* pfl = page_free_list;
	struct Page* last_pfl = NULL;
	while (pfl != NULL)
	{
		struct Page* tem = pfl;
		int flag = 0;
		for (i=0; i<n-1; ++i)
		{
			if (tem->pp_link == NULL) {flag = 1; break;}
			if ((tem - tem->pp_link) != 1) {flag = 1; break;}
			tem = tem->pp_link;
		}
		...
		else
		{
			...
			pfl = tem;
			if (last_pfl == NULL)
				page_free_list = tem->pp_link;
			else
				last_pfl->pp_link = tem->pp_link;
...
page_free_npages(struct Page *pp, int n)
{
	...
	struct Page* tem = pp;
	struct Page* last_tem = NULL;
	int i;
	for (i=0; i<n; ++i)
	{
		last_tem = tem->pp_link;
		page_free(tem);
		tem = last_tem;
	}
	return 0;
...
page_free(struct Page *pp)
{
	...
	{
		while (pp < (tem->pp_link))
			tem = tem->pp_link;
		pp->pp_link = tem->pp_link;
		tem->pp_link = pp;
	}
}
//
...
page_realloc_npages(struct Page *pp, int old_n, int new_n)
{
	...
	if (old_n > new_n)
	{
		for (i=0; i<new_n; ++i) tem = tem->pp_link;
		page_free_npages(tem, old_n - new_n);
		return pp;
	} 
	for (i=0; i<old_n-1; ++i) tem = tem->pp_link;
	struct Page *target = tem + new_n - old_n;
	struct Page *pfl = page_free_list;
	struct Page *last_pfl = NULL;
	while (pfl > pp)
	{
		if (pfl == target)
		{
			struct Page* temp = pfl;
			int flag = 0;
			for (i=0; i<(new_n-old_n-1); ++i)
			{
				if (temp->pp_link == NULL) {flag = 1; break;}
				if ((temp - temp->pp_link) != 1) {flag = 1; break;}
				temp = temp->pp_link;
			}
			if (flag == 0)
			{
				if (page_free_list == target)
					page_free_list = temp->pp_link;
				else
					last_pfl->pp_link = temp->pp_link;
	...
	struct Page *new_list = page_alloc_npages(ALLOC_ZERO, new_n);
	memmove(page2kva(new_list),page2kva(pp),old_n*PGSIZE);
...
page_lookup(pde_t *pgdir, void *va, pte_t **pte_store)
{
	...
	if (PTE_P & (*pte)) return pa2page(PTE_ADDR(*pte));
	return NULL;
}
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	for(p = ROUNDDOWN((unsigned int*)va, PGSIZE); p < ROUNDUP((unsigned int*)(va + len), PGSIZE); p += PGSIZE){
		pte_t* pte_p = pgdir_walk(env->env_pgdir, p, 0);
		if(p > (unsigned int*)ULIM ||	//out of range
			!pte_p ||	//page doesn't exist
			((*pte_p) & perm) != perm)	//less perm than expected.
		{
			//sth is wrong.
			user_mem_check_addr = ((uintptr_t)va>(uintptr_t)p)?(uintptr_t)va:(uintptr_t)p;	//for buggyhello2
			return -E_FAULT;
		}
	}	
	return 0;
...
check_page_free_list(bool only_low_memory)
{
	...
	cprintf("Pass check_page_free_list\n");
}
